...
function applyFiltersToSeed(list: Listing[], f: BrowseFilters): Listing[] {
  return list
    .filter((l) => {
    ...
    .sort((a, b) => a.postedDaysAgo - b.postedDaysAgo); // newest first
}

function seedPage(filters: BrowseFilters, page: number, pageSize: number): BrowsePage {
  const filtered = applyFiltersToSeed(seedListings, filters);
...
export async function getBrowsePage(
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL) {
    return seedPage(filters, safePage, pageSize);
  }
